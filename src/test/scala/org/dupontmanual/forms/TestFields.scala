...
class TestFields extends FunSuite {
  ...
  test("1. NumericField[Int]") {
    ...
    assert(f.clean("a") === Left(ValidationError("This value must be a positive or negative whole number (Int).")))
    assert(f.clean("3.14") === Left(ValidationError("This value must be a positive or negative whole number (Int).")))
    assert(f.clean("1 ") === Right(1))
    assert(f.clean(" 2") === Right(2))
    assert(f.clean(" 3 ") === Right(3))
    assert(f.clean("4a") === Left(ValidationError("This value must be a positive or negative whole number (Int).")))
  ...
  test("1. required NumericField[Double]") {
    ...
    assert(f.clean("a") === Left(ValidationError("This value must be a number (Double).")))
    assert(f.clean("3.14") === Right(3.14))
    assert(f.clean("-1.5 ") === Right(-1.5))
    assert(f.clean(" 2.5") === Right(2.5))
    assert(f.clean(" -2.5 ") === Right(-2.5))
    assert(f.clean("    ") === Left(ValidationError("This field is required.")))
    assert(f.clean("4a") === Left(ValidationError("This value must be a number (Double).")))
    assert(f.maxValue === None)
    assert(f.minValue === None)
  }
